...
However the Zoom H2E can record to two separate mid side raw stereo files. One using the front mic as the mid and the other using the rear mic as the mid. 

This is called Double Mid Side and can be used with a plugin like [ab Encoder MS](https://www.audiobrewers.com/tech-support/audio-brewers-ab-encoder-ms) to turn it into a Ambisonic AmbiX format. This plugin is $15 and does a great job!

Once in a Ambisonic format you can use the audio with plugins like Rode Soundfield(My favorite) to rotate, change mic pickup patterns, and refocus the audio or dearVR Ambi Micro or any other number of Ambisonic binuaral encoder plugins to turn it in to 3d sounding binaural audio. 

The Ambisonic format is incredibly useful and really makes the Zoom H2E a pocketable swiss army knife of recorders!
